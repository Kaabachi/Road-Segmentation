...
class RoadsDatasetTrain(Dataset):
    ...
    def __len__(self):
        return int(self.number_patch_per_image * len(self.img_names))
    ...
    def __getitem__(self, idx):
        ...
        image_index = int(idx // n_p_p_i)
        patch_index = int(idx % n_p_p_i)
        ...
        small_image, small_groundtruth = self._get_patch(
            padded_image, padded_groundtruth, patch_index
        )
    ...
    def _get_patch(self, image, groundtruth, patch_number):
        ...
        image_patch = image[
            x - padding : x + l_p_s - padding, y - padding : y + l_p_s - padding
        ]

        groundtruth_patch = groundtruth[
            x - padding : x + l_p_s - padding, y - padding : y + l_p_s - padding
        ]
