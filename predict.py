...
def save_image(image):
    ...
    mask = torch.tensor(image, dtype=torch.float)
...
#     indices = np.zeros((len(dataloader), 3), dtype=int)
    ...
    tmp_img = np.zeros((IMAGE_SIZE, IMAGE_SIZE))
...
#         indices[ind_batch] = np.array([sample_batched['id'], sample_batched['x'], sample_batched['y']])
        ...
        tmp = np.array(final[0][0])
        small_patch = crop(tmp)
        ...
        tmp_img[image_number, start_x:end_x, start_y:end_y] = small_patch
        
        if index[1] == NUMBER_PATCHES_PER_IMAGE - 1 && index[2] == NUMBER_PATCHES_PER_IMAGE - 1:
            save_image(tmp_img)
